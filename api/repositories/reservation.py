...
from api.models.reservation import Reservation, ReservationCreate, ReservationUpdate, ReservationUpdateHolder, ReservationUpdateStatus, StatusEnum
...
class ReservationRepository:
    ...
    def get_all_available_by_room(self, room_number: int) -> list[Reservation]:
        return self.session.exec(select(Reservation).where(Reservation.room_number == room_number, Reservation.status != StatusEnum.cancelled)).all()
    ...
    def update_status(self, reservation_db: Reservation, reservation_status: ReservationUpdateStatus) -> Reservation | None:
        reservation_db.sqlmodel_update(reservation_status)
        self.session.add(reservation_db)
        self.session.commit()
        self.session.refresh(reservation_db)
        return reservation_db
    
    def update_holder(self, reservation_db: Reservation, new_holder_id: int) -> Reservation | None:
        reservation_db.user_id = new_holder_id
        self.session.add(reservation_db)
        self.session.commit()
        self.session.refresh(reservation_db)
        return reservation_db
