...
from api.utils.security.dependencies import company_user, session_user

from api.service.notification import create_noti, get_noti_by_id, get_all_noti, get_all_noti_by_user, delete_noti

router = APIRouter(prefix="/notifications", tags=["notifications"])

@router.post("", response_model=Notification, status_code=status.HTTP_201_CREATED)
async def create_notification(notification_data: NotificationCreate, repo: notification_repo, _: company_user):
...
@router.get("/me", response_model=list[Notification], status_code=status.HTTP_200_OK)
async def get_my_notifications(repo: notification_repo, payload: session_user):
    return get_all_noti_by_user(payload["id"], repo)
